...
export default function DashboardPage() {
  ...
  const fetchTasks = async (uid: string) => {
    try {
      ...
      const apiUrl =
        ...
        "https://full-stack-todo-app-hackathon-phase-ii-hb5r.onrender.com";

      console.log("Fetching tasks for user:", uid);
      console.log("Token exists:", !!token);
      console.log("API URL:", `${apiUrl}/api/users/${uid}/tasks`);

      // CORRECT SYNTAX - parenthesis around the URL
  ...
  const toggleComplete = async (taskId: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/users/${userId}/tasks/${taskId}/complete`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const deleteTask = async (taskId: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/users/${userId}/tasks/${taskId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
